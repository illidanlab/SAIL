...
class FeedForwardPolicy(TD3Policy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, reuse=False, layers=None,
        ...
        # self.bc_predict_ph = tf.placeholder(dtype=ac_space.dtype, shape=(n_batch,) + ac_space.shape,
        #                                          name="bc_predict_ph")
    ...
    def make_bcq_actor(self, obs=None, action=None, reuse=False, scope="pi", threshold=0.05): 
        with tf.variable_scope(scope, reuse=reuse):
            if self.feature_extraction == "cnn":
                obs = self.cnn_extractor(obs, **self.cnn_kwargs)
            else:
                obs = tf.layers.flatten(obs)

            act = tf.layers.flatten(action)
            input_ph = tf.concat([obs, act], axis=-1)
            a = mlp(input_ph, self.layers, self.activ_fn, layer_norm=self.layer_norm)
            a = threshold * tf.layers.dense(a, self.ac_space.shape[0], activation=tf.tanh)
            policy = action + a # perturbate action
    ...
    def make_dual_critics(self, obs=None, action=None, reuse=False, scope="values_fn"):
        if obs is None:
            obs = self.processed_obs
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            with tf.variable_scope('qf1', reuse=reuse):
                ...
                qf1_dual = tf.layers.dense(qf1_h, 1, name="qf1_dual")

            with tf.variable_scope('qf2', reuse=reuse):
                qf2_h = mlp(qf_h, self.layers, self.activ_fn, layer_norm=self.layer_norm)
                qf2 = tf.layers.dense(qf2_h, 1, name="qf2")
                qf2_dual = tf.layers.dense(qf2_h, 1, name="qf2_dual")

            self.qf1 = qf1
            self.qf1_dual = qf1_dual
            self.qf2 = qf2
            self.qf2_dual = qf2_dual

        return self.qf1, self.qf1_dual, self.qf2, self.qf2_dual

    def make_dice_critics(self, obs, action, reuse=False, scope="dice_values_fn"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            with tf.variable_scope('qf1', reuse=reuse):
                qf1_h = mlp(qf_h, self.layers, self.activ_fn, layer_norm=self.layer_norm)
                qf  = tf.layers.dense(qf1_h, 1, name="qf1") 
        return qf
    ...
    def bcq_step(self, obs, feed_dict):
        feed_dict[self.obs_ph] = obs
        action = self.sess.run(self.policy, feed_dict)
        return action
