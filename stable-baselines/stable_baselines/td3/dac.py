...
class DAC(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000, demo_buffer_size=1e4,
                 ...
                 behavior_clone=False, q_regularize=False, early_stop=False, bc_expert=False, absorbing_per_episode=1,using_return=False,
        ...
        self.train_discriminator_freq = 500#train_freq // 2
        self.hidden_size_adversary = 256#100
    ...
    def mmd_loss_laplacian(self, samples1, samples2, sigma=10):
        ...
        if self.bc_expert:
            overall_loss = self.if_demo_ph * overall_loss
        return overall_loss
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.mmd_loss_decay_ph = tf.placeholder(tf.float32, [], name='mmd_loss_decay_ph')
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    if self.behavior_clone:
                        ...
                        # mmd_loss_decay_ph is used for 'early stop' on support constraint
                        policy_loss = tf.reduce_mean( -qf1_pi + self.mmd_loss_decay_ph * tf.exp(self.bc_log_lagrange) * mmd_losses)
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        if self.behavior_clone: 
            ...
            feed_dict[self.mmd_loss_decay_ph] = float(self.mmd_loss_decay)
            if self.bc_expert:
                feed_dict[self.if_demo_ph] = batch_if_demos 
    ...
    def generate_discriminator_data(self, mode = None):
        if mode and 'synthetic' in mode:
            ...
            # generate self-generated data
            batch = self.replay_buffer.sample(self.d_batch_size // 2)
            # for states visited by experts, generate actions predicted by the policy, treat them as self-generated data
            fake_ob_batch = self.demo_replay_buffer.sample(self.d_batch_size // 2)[0]
            fake_ac_batch = self.predict(fake_ob_batch)[0] 
            ob_batch, ac_batch =  np.concatenate((batch[0], fake_ob_batch)), np.concatenate((batch[1], fake_ac_batch))
        else:
            expert_batch = self.demo_replay_buffer.sample(self.d_batch_size )
            ob_expert, ac_expert = expert_batch[:2]
            # disturb expert actions if needed
    ...
    def initialize_expert_buffer(self):
        ...
        # get episode_score for each demo sample 
        ...
        for idx in range(n_samples-1):
            ...
            if demo_dones[idx] == 0 and demo_dones[idx+1] == 1:
            ...
            self.demo_replay_buffer.add(demo_obs[idx], demo_actions[idx], demo_rewards[idx], demo_next_obs[idx], float(demo_dones[idx]), 1.0, episode_score, demo_rewards[idx])
            if idx % 500 == 0:
                print("Adding demonstration to the replay buffer, processing {} %  ..".format(float(idx+1) * 100 / n_samples))
        self.demo_replay_buffer.add(demo_obs[-1], demo_actions[-1], demo_rewards[-1], demo_next_obs[-1], float(demo_dones[-1]), 1.0, episode_score, demo_rewards[-1])
    ...
    def learn(self, total_timesteps, callback=None, seed=None, test_env=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                ##############################################
                truncated_done = done and episode_timesteps + 1 == self.env.max_episode_steps  # pylint: disable=protected-access

                if done:
                    if not truncated_done:
                        #print("Step {} /{}, Change to absorbing states ....".format(episode_timesteps+1, self.env.max_episode_steps))
                        new_obs = self.env.envs[0].get_absorbing_state()
                    #else:
                        #print("Step {} /{}. Episode ends due to max-episode-step reached.".format(episode_timesteps+1, self.env.max_episode_steps))
                ##############################################
                # add transition to buffer
                ############################################## 
                self.episode_buffer.add(obs, action, reward, new_obs, float(done), 0, episode_score, true_reward) 
                self.replay_buffer.add(obs, action, reward, new_obs, float(done),  0, episode_score, 0)
                ############################################## 
                ...
                if done and not truncated_done:
                    for abs_i in range(self.absorbing_per_episode):
                        if abs_i + episode_timesteps < self.env.max_episode_steps:  # pylint: disable=protected-access
                            ...
                            self.replay_buffer.add(obs, action, 0, new_obs, 1.0,  0, episode_score, 0)
                ...
                if episode_score is not None:
                    ...
                    if self.adaptive:
                        if episode_score > self.expert_scores[0]:
                            ...
                            ## decay mmd-loss support constraint -- Judy 
                            if self.early_stop:
                                self.mmd_loss_decay = 0.0
                            for transition in self.episode_buffer.get_episode():
                                s, a, r, s1, if_done, if_demo, scores, true_r = transition
                                self.demo_replay_buffer.add(s, a, r, s1, if_done, 1.0, episode_score, true_r)
                    ...
                    #        for transition in self.episode_buffer.get_episode():
                    #            s, a, r, s1, if_done, if_demo, scores, true_r = transition
                    #            self.replay_buffer.add(s, a, r, s1, if_done, 0, episode_score, true_r)
                    #else:
                ...
                if test_env is not None and step % self.eval_freq == 0:
                    self.eval(test_env, deterministic=True, episodes=1, render=False, callback=None)
