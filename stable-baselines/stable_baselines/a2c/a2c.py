...
class A2C(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=2, vf_coef=0.25, ent_coef=0.01, max_grad_norm=0.5,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.proba_step_with_temperature = step_model.proba_step_with_temperature
    ...
    def learn_from_rewardshaping(
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            runner = A2CRewardShapingRunner(self.env, self, expert_model=expert_model, n_steps=self.n_steps, gamma=self.gamma)
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1000, tb_log_name="A2C",
              reset_num_timesteps=True):
...
class A2CRunner(AbstractEnvRunner):
    def __init__(self, env, model, expert_model=None, run_rs=False, n_steps=5, gamma=0.99):
        """
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            #    return a - np.max(a)
            #if self.run_rs:
            #    action_prob_for_shaping = self.expert_model.action_probability(obs)
            #    #action_prob_for_shaping = self.expert_model.action_probability_with_temperature(obs)
            #    # get logit distribution (in each environment)
            #    action_prob_for_shaping = np.apply_along_axis(remove_max, 1, action_prob_for_shaping)
            #    # get logit value of the selected action (in each environment)
            #    logits = np.array([action_prob_for_shaping[i,j] for i, j in enumerate(clipped_actions)])
            #    #rewards += logits
            ########################### 
...
class A2CRewardShapingRunner(AbstractEnvRunner):
    def __init__(self, env, model, expert_model=None, n_steps=5, gamma=0.99):
        """
        A runner to learn the policy of an environment for an a2c model
        ...
        :param gamma: (float) Discount factor
        """
        super(A2CRewardShapingRunner, self).__init__(env=env, model=model, n_steps=n_steps)
        self.gamma = gamma
        self.expert_model = expert_model
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            self.obs = obs
            mb_rewards.append(rewards)
            mb_logits.append(logits) ## save logit value to minibatch

        mb_dones.append(self.dones)
        # batch of steps to batch of rollouts
        mb_obs = np.asarray(mb_obs, dtype=self.obs.dtype).swapaxes(1, 0).reshape(self.batch_ob_shape)
        mb_rewards = np.asarray(mb_rewards, dtype=np.float32).swapaxes(0, 1)
        mb_logits = np.asarray(mb_logits, dtype=np.float32).swapaxes(0, 1)
        mb_actions = np.asarray(mb_actions, dtype=self.env.action_space.dtype).swapaxes(0, 1)
        mb_values = np.asarray(mb_values, dtype=np.float32).swapaxes(0, 1)
        mb_dones = np.asarray(mb_dones, dtype=np.bool).swapaxes(0, 1)
        mb_masks = mb_dones[:, :-1]
        ...
        last_values = self.model.value(self.obs, self.states, self.dones).tolist()
        # get last logit value
        action_prob_for_shaping = self.expert_model.action_probability(self.obs)
        action_prob_for_shaping = np.apply_along_axis(remove_max, 1, action_prob_for_shaping)
        actions, _, _, _ = self.model.step(self.obs, self.states, self.dones)
        clipped_actions = actions
        # Clip the actions to avoid out of bound error
        if isinstance(self.env.action_space, gym.spaces.Box):
            clipped_actions = np.clip(actions, self.env.action_space.low, self.env.action_space.high)
        last_logits = np.array([action_prob_for_shaping[i,j] for i, j in enumerate(clipped_actions)]).tolist()

        # discount/bootstrap off value fn
        for n, (logits, rewards, dones, value) in enumerate(zip(mb_logits, mb_rewards, mb_dones, last_values)):
            rewards = rewards.tolist()
            logits = logits.tolist()
            dones = dones.tolist()
            if dones[-1] == 0:
                rewards = discount_with_dones(rewards + [value], dones + [0], self.gamma, logits=logits + last_logits)[:-1]
            else:
                rewards = discount_with_dones(rewards, dones, self.gamma, logits=logits)
            mb_rewards[n] = rewards
