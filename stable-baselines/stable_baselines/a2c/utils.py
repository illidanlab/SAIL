...
def discount_with_dones(rewards, dones, gamma, logits=None):
    """
    ...
    if logits != None:
        #print(len(logits), len(rewards), len(dones))
        for logit, reward, done in zip(logits[::-1], rewards[::-1], dones[::-1]):
            # ret is Q estimate for old MDP
            ret = reward + gamma * ret * (1. - done)  # fixed off by one bug
            ret2send = np.clip(ret + logit, -1, 1)  
            # ret + logit is Q estimate for new MDP
            discounted.append(ret2send)
    else:
...
def total_episode_logit_reward_logger(rew_acc, logit_acc, rewards, logits, masks, writer, steps):
    """
    calculates the cumulated episode reward, and prints to tensorflow log the output

    :param rew_acc: (np.array float) the total running reward
    :param logit_acc: (np.array float) the mean running logits
    :param rewards: (np.array float) the rewards
    :param logits: (np.array float) the logits
    ...
    with tf.variable_scope("environment_info", reuse=True):
        for env_idx in range(rewards.shape[0]):
            ...
            if len(dones_idx) == 0:
                ...
                logit_acc[env_idx] += np.mean(logits[env_idx])
            else:
                rew_acc[env_idx] += sum(rewards[env_idx, :dones_idx[0, 0]])
                logit_acc[env_idx] += np.mean(logits[env_idx, :dones_idx[0, 0]])
                ...
                logit_summary = tf.Summary(value=[tf.Summary.Value(tag="episode_logit", simple_value=logit_acc[env_idx])])
                writer.add_summary(logit_summary, steps + dones_idx[0, 0])
                for k in range(1, len(dones_idx[:, 0])):
                    rew_acc[env_idx] = sum(rewards[env_idx, dones_idx[k-1, 0]:dones_idx[k, 0]])
                    summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward", simple_value=rew_acc[env_idx])])
                    writer.add_summary(summary, steps + dones_idx[k, 0])
                    logit_acc[env_idx] = np.mean(logits[env_idx, dones_idx[k-1, 0]:dones_idx[k, 0]])
                    logit_summary = tf.Summary(value=[tf.Summary.Value(tag="episode_logit", simple_value=logit_acc[env_idx])])
                    writer.add_summary(logit_summary, steps + dones_idx[k, 0])
                rew_acc[env_idx] = sum(rewards[env_idx, dones_idx[-1, 0]:])
                logit_acc[env_idx] = np.mean(logits[env_idx, dones_idx[-1, 0]:])

    return rew_acc, logit_acc
