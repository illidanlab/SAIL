...
def build_train_lfd(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
                gamma=1.0, double_q=True, scope="deepq", reuse=None, regularization=0.1,
    ...
    :return: (tuple)
        ...
        pretrain: (function (Any, numpy float, numpy float, Any, numpy bool, numpy float): numpy float)
            optimize the supervise-loss: J_E(Q) = max_a[Q(s,a) + l(a_E, a)] - Q(s, a_E)
            check https://arxiv.org/pdf/1704.03732.pdf for details.
    ...
    with tf.variable_scope("loss", reuse=reuse):
        ...
        demo_t_ph = tf.placeholder(tf.float32, [None], name="demo")
        ...
        ###########################
        # 1. compute J_E loss: max_a[Q(s,a) + l(a, a_E)] - Q(s,a), where Q() is the selection model --Judy
        ###########################
        one_hot_action = tf.one_hot(act_t_ph, n_actions) 
        margin_value = (tf.ones_like(one_hot_action) - one_hot_action ) * 0.8 
        # only demonstration transitions will have margin loss
        J_E = demo_t_ph * ( tf.reduce_max(margin_value + step_model.q_values, axis=1) - q_t_selected )

        # compute estimate of best possible value starting from state at t + 1
        if double_q:
            q_tp1_best_using_online_net = tf.argmax(double_q_values, axis=1) # use double-net to select best action -- Judy
            q_tp1_best = tf.reduce_sum(target_policy.q_values * tf.one_hot(q_tp1_best_using_online_net, n_actions), axis=1) # use target-net to estimate the Q value
        else:
        ...
        ###########################
        # 2. compute J_Q loss:  the td-error (potentially clipped)
        ###########################
        # TBD: N-step td error
        td_error = q_t_selected - tf.stop_gradient(q_t_selected_target)

        ###########################
        # 3. compute the L2 loss: J_reg
        ###########################
        q_reg_vars = [var for var in q_func_vars if 'bias' not in var.name]
                               #if 'bias' not in var.name and 'qf_output' not in var.name and 'b' not in var.name]
        
        J_reg = tc.layers.apply_regularization(
            tc.layers.l2_regularizer(scale=regularization),
            weights_list=q_reg_vars
            )

        ###########################
        # calculate the weighted loss
        ###########################
        td_loss = tf_util.huber_loss(td_error)
        total_loss = tf_util.huber_loss(td_error) + 1e2 * J_E +  1e-3 * J_reg
        #total_loss = J_E
        #total_loss = tf_util.huber_loss(td_error)

        weighted_error = tf.reduce_mean(importance_weights_ph * td_loss)
        weighted_total_error = tf.reduce_mean(importance_weights_ph * total_loss)
        tf.summary.scalar("weighted_loss", weighted_error)
        tf.summary.scalar("weighted_total_error", weighted_total_error)
        tf.summary.scalar("td_error", tf.reduce_mean(td_error))
        tf.summary.scalar("J_E", tf.reduce_mean(J_E))
        ...
        gradients = optimizer.compute_gradients(weighted_total_error, var_list=q_func_vars)
