...
class CategoricalProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_dual_latent(self, pi_latent_vector, vf_latent_vector, true_vf_latent_vector, init_scale=1.0, init_bias=0.0):
        pdparam = linear(pi_latent_vector, 'pi', self.n_cat, init_scale=init_scale, init_bias=init_bias)
        q_values = linear(vf_latent_vector, 'q', self.n_cat, init_scale=init_scale, init_bias=init_bias)
        true_q_values = linear(true_vf_latent_vector, 'true_q', self.n_cat, init_scale=init_scale, init_bias=init_bias)
        return self.proba_distribution_from_flat(pdparam), pdparam, q_values, true_q_values
...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_dual_latent(self, pi_latent_vector, vf_latent_vector, true_vf_latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
        q_values = linear(vf_latent_vector, 'q', self.size, init_scale=init_scale, init_bias=init_bias)
        true_q_values = linear(true_vf_latent_vector, 'true_q', self.size, init_scale=init_scale, init_bias=init_bias)
        return self.proba_distribution_from_flat(pdparam), mean, q_values, true_q_values
