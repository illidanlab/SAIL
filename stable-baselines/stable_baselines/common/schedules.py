...
class BlockwiseSchedule(Schedule):
    def __init__(self, schedule_timesteps, final_p, initial_p=1.0, blocksize=1000, meta_lr=None):
        # drop learning rate per 'block' steps
        self.schedule_timesteps = schedule_timesteps
        self.final_p = final_p
        self.initial_p = initial_p
        self.blocksize = blocksize
        self.block = schedule_timesteps // blocksize 
        if meta_lr is not None:
            self.meta_lr = meta_lr
        else:
            self.meta_lr = (final_p - initial_p) // self.block

    def value(self, step):
        block_id = step // self.blocksize
        #fraction = min(float(step) / self.schedule_timesteps, 1.0)
        return max(1e-8, self.initial_p + block_id * self.meta_lr)
