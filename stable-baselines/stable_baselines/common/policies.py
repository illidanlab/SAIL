...
def mlp_dual_extractor(flat_observations, net_arch, act_fun):
    """
    Similar to mlp_extractor, but output two value functions, one for the MDP with real-reward, and one for the MDP with sythetic reward. 
    """
    latent = flat_observations
    policy_only_layers = []  # Layer sizes of the network that only belongs to the policy network
    value_only_layers = []  # Layer sizes of the network that only belongs to the value network
    true_value_only_layers = []  # Layer sizes of the network that only belongs to the true-value network
    ...
    for idx, layer in enumerate(net_arch):
        if isinstance(layer, int):  # Check that this is a shared layer
            ...
            if 'vf' in layer:
                ...
                value_only_layers = layer['vf']
            if 'true_vf' in layer:
                assert isinstance(layer['true_vf'], list), "Error: net_arch[-1]['true_vf'] must contain a list of integers."
                true_value_only_layers = layer['true_vf']
            break  # From here on the network splits up in policy and value network

    # Build the non-shared part of the network
    latent_policy = latent
    latent_value = latent
    latent_true_value = latent
    for idx, (pi_layer_size, vf_layer_size, true_vf_layer_size) in enumerate(zip_longest(policy_only_layers, value_only_layers, true_value_only_layers)):
        if pi_layer_size is not None:
            assert isinstance(pi_layer_size, int), "Error: net_arch[-1]['pi'] must only contain integers."
            latent_policy = act_fun(linear(latent_policy, "pi_fc{}".format(idx), pi_layer_size, init_scale=np.sqrt(2)))
        ...
        if true_vf_layer_size is not None:
            assert isinstance(true_vf_layer_size, int), "Error: net_arch[-1]['true_vf'] must only contain integers."
            latent_true_value = act_fun(linear(latent_true_value, "true_vf_fc{}".format(idx), true_vf_layer_size, init_scale=np.sqrt(2)))

    return latent_policy, latent_value, latent_true_value
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, temperature=1, reuse=False, scale=False):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution):
                ...
                self._policy_proba_with_temperature = tf.nn.softmax(self.policy * self.temperature)
    ...
    def _setup_init_dual_critic(self):
        """
        similar to _setup_init, but for two critics. --Judy 
        """
        with tf.variable_scope("output", reuse=True):
            assert self.policy is not None and self.proba_distribution is not None and self.value_fn is not None and self.true_value_fn is not None 
            self._action = self.proba_distribution.sample()
            self._deterministic_action = self.proba_distribution.mode()
            self._neglogp = self.proba_distribution.neglogp(self.action)
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution):
                ...
                self._policy_proba = []  # it will return nothing, as it is not implemented
            self._value_flat = self.value_fn[:, 0]
            self._true_value_flat = self.true_value_fn[:, 0]
    ...
    @property
    def true_value_fn(self):
        """tf.Tensor: value estimate, of shape (self.n_batch, 1)"""
        return self._true_value_fn

    @property
    def true_value_flat(self):
        """tf.Tensor: value estimate, of shape (self.n_batch, )"""
        return self._true_value_flat
    ...
    @property
    def policy_proba_with_temperature(self):
        """tf.Tensor: parameters of the probability distribution. Depends on pdtype."""
        return self._policy_proba_with_temperature
...
class DualFeedForwardPolicy(ActorCriticPolicy):
    """
    Similar to FeedForwardPolicy, but have two critics: one for MDP with real reward, another for MDP with sythetic reward. 
    """

    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, temperature=1, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", **kwargs):
        super(DualFeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, temperature=temperature, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                raise ValueError("Dual critic for reward shaping is not implemented for CNN policy yet -- Judy")
            else:
                pi_latent, vf_latent, vf_true_latent = mlp_dual_extractor(tf.layers.flatten(self.processed_obs), net_arch, act_fun)

            self._value_fn = linear(vf_latent, 'vf', 1)
            self._true_value_fn = linear(vf_true_latent, 'true_vf', 1)

            #self._proba_distribution, self._policy, self.q_value = \
            #    self.pdtype.proba_distribution_from_latent(pi_latent, vf_latent, init_scale=0.01)
            self._proba_distribution, self._policy, self.q_value, self._true_q_value = \
                self.pdtype.proba_distribution_from_dual_latent(pi_latent, vf_latent, vf_true_latent, init_scale=0.01)

        #self._setup_dual_init()
        self._setup_init_dual_critic()

    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, true_value, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.true_value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
        else:
            action, value, true_value, neglogp = self.sess.run([self.action, self.value_flat, self.true_value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
        return action, value, true_value, self.initial_state, neglogp

    def proba_step(self, obs, state=None, mask=None):
        return self.sess.run(self.policy_proba, {self.obs_ph: obs})

    def proba_step_with_temperature(self, obs, state=None, mask=None):
        return self.sess.run(self.policy_proba_with_temperature, {self.obs_ph: obs})

    def value(self, obs, state=None, mask=None):
        return self.sess.run(self.value_flat, {self.obs_ph: obs})

    def true_value(self, obs, state=None, mask=None):
        return self.sess.run(self.true_value_flat, {self.obs_ph: obs})

    def true_q_value(self, obs, actions, state=None, mask=None):
        return self.sess.run(self._true_q_value,  {self.obs_ph: obs, self.action_ph: actions})
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, temperature=1, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", **kwargs):
        super(FeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, temperature=temperature, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))
...
class CnnPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, temperature=1, reuse=False, **_kwargs):
        super(CnnPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse, temperature=temperature,
...
class DualMlpPolicy(DualFeedForwardPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(DualMlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
